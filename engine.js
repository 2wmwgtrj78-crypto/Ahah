...
var DEFAULTS = {
  /* One day model. 9 h 15 m of study is the Normal day; Empathy is the same day
     ...
     tiers are gone. The campaign starts 20 September 2026, a Sunday, with an
     ORIENTATION day: no syllabus content, no slots, no arrears \u2014 set the app
     up, look at the map, log a handful of questions to seed the evidence.
     Content begins Monday 21 Sep, and the first rampDays content days are SOFT
     (one slot instead of two), so week one is genuinely lighter and the
     campaign extends forward to absorb it rather than running a deficit. */
  ...
  p2At: 0.20, p3At: 0.40,
  empathyHours: 7.25,
  /* Low is the third day model: 3 h 15 m. Empathy is a shorter serious day;
     Low is for a day that is NOT yours — a night on call, illness, travel,
     a family day. It exists so those days produce a real, finishable block
     instead of a 7h15m day you abandon at minute forty and log as nothing.
     Chosen, never triggered, like the other two. Same shaping as a ramp day:
     lectures go first and questions keep their analysis. */
  lowHours: 3.25,
  ...
  /* A deliberate ramp for the first week: MCQs and their analysis only, at a
     length you can actually finish. Lectures are cut first by CUT_FIRST, so a
     3-hour target naturally leaves questions plus review — the work that
     produces the accuracy data everything else adapts to. Set rampDays to 0 to
     start at full load.

     v15.3: the ramp used to exist ONLY in shapeToday, which trimmed the day on
     screen while buildPlan had already allocated a full day of slots to it.
     The difference did not vanish — it became arrears, so a deliberately
     gentle first week opened by generating hours of self-inflicted backlog.
     The allocator now reads rampDays too: those days are soft (one slot, not
     two), the skeleton grows to fit, and the schedule moves forward instead. */
  rampDays: 7, rampHours: 5,
  /* rampHours is a CEILING the shaper applies on top of the soft allocation,
     not a second, competing target. It was 3 h while the ramp was cosmetic,
     and leaving it there would have re-created exactly the arrears this change
     removes: the soft week now allocates 2 h 36 m to 5 h a day, so a 3 h cap
     would trim four of the seven days and bank the difference as backlog. At
     5 h it binds on the heaviest soft day only. */
  /* Day one is orientation, not study. It is a rest-kind day: it carries no
     syllabus slots, so it cannot create debt or displace a single question. */
  orientStart: true,
  /* Third pass on every topic (0 = no yield gate). Raising this to 1 or 2 drops
     the third pass from the lowest-yield topics — a trade the Fit screen
     prices in hours before it is taken, never something applied silently. */
  p3MinYield: 0,
  /* Phase retention: every Nth content day, the day's closing blank-page recall
     points at a COMPLETED phase instead of today's material. Costs nothing —
     the block already existed — and it is the only thing standing between
     Phase 1 and ~208 untouched days before the paper. 0 disables it. */
  retentionEvery: 3,
  ...
  diagnosticDays: 14,
  startISO: "2026-09-20", hardStartISO: null, correctEvery: 14,
...
function budget(P, contentDays, slotCount){
  ...
  /* Per-topic pass count. The third pass is yield-gated (p3MinYield, 0 by
     default = every topic, i.e. the hard three-pass rule). Because the gate is
     per topic, the repeat pace and the analysis rate have to be summed per
     topic too — a single global `rep` would price a pass the allocator is
     not going to schedule, and the plan would claim to fit on hours it never
     spends. */
  function passesFor(t){ return (nP>=3 && (Number(P.p3MinYield)||0) <= Number(t.yINI||0)) ? nP : Math.min(nP,2); }
  function repFor(t){ var n=passesFor(t), r=0; for(var pi=1; pi<n; pi++) r+=PASS_PACE[Math.min(pi,2)]; return r; }
  function anaFor(t){ return passFactors(passesFor(t)).a; }
  var Qw = CURRICULUM.reduce(function(a,t){
    return a + t.dtq*P.bankPace + t.spmin + (t.dtq+t.spq)*P.bankPace*repFor(t); },0);
  ...
  var A = CURRICULUM.reduce(function(a,t){ return a+(t.dtq+t.spq)*(P.anaPerQ||1.2)*anaFor(t)*anaShare(P,t.i); },0), warn = null;
  ...
  var topics = CURRICULUM.map(function(t){
    ...
    /* Third pass, yield-gated. Three passes over everything is the hard rule
       and p3MinYield stays at 0 by default, so nothing changes unless the user
       deliberately trades it in the Fit screen against an arithmetic gap they
       can see. When they do, the pass that gets dropped is dropped from the
       LOWEST-yield topics first rather than from wherever the calendar happens
       to run out — which is what actually happens if the plan is simply
       allowed to overrun the exam. */
    o.p3 = passesFor(t)>=3;
    o.re3Q = o.p3 ? t.dtq+t.spq : 0;
    o.work = lecEff + t.dtq*P.bankPace + t.spmin
           + (t.dtq+t.spq)*P.bankPace*repFor(t) + (t.dtq+t.spq)*(P.anaPerQ||1.2)*anaFor(t)*anaShare(P,t.i);
    return o;
  });
...
function skeleton(P, neededContent){
  ...
  while((nContent<neededContent || d<=lastExam) && guard++<4000){
    ...
    if(onExam) row={date:sd,rest:true,kind:"exam",exam:onExam};
    ...
    else if(P.orientStart && sd===P.startISO) row={date:sd,rest:true,kind:"orient"};
      ...
      /* Soft-start days carry one slot instead of two. The allocator already
         understood `soft` (effDays counts them as half, slotCount as one) —
         it was simply hardcoded false, which made the ramp cosmetic. */
      row={date:sd,rest:false,kind:"content",soft:nContent<(P.rampDays||0)};
      nContent++; cSince++;
    }
    days.push(row);
...
/* A day's slot ceiling. Soft-start days hold ONE slot; everything else holds
   up to three. Without this, level() and balance() immediately undid the soft
   start: they push slots onto any day under capacity, and a 1-slot opening day
   is the most under-capacity day in the campaign, so week one refilled itself
   to 7-8 hours within a pass or two. */
function slotCap(c){ return c && c.soft ? 1 : 3; }
  ...
  for(var pass=0; pass<120; pass++){
    ...
    for(var oi=0; oi<order.length; oi++){
      ...
      for(var j=Math.max(0,i-25); j<Math.min(cd.length,i+26); j++){
        if(j===i) continue;
        var d=cd[j];
        if(d.slots.length>=slotCap(d)) continue;
...
function balance(cd,P,B){
  ...
  function canTake(c,s){
    return c.slots.length<slotCap(c)
...
function dayBlocks(day,P,B){
  ...
  /* Phase retention thread. Under strict phase-lock, Phase 1 finishes early
     and is then not opened again for roughly 208 days before the paper —
     long enough that the first pass is effectively spent. Every retentionEvery
     content days this closing block is RE-POINTED at a completed phase instead
     of today's material. It adds no minutes: the block already existed, and
     retrieval of old material at the end of the day is worth more than
     retrieval of material you saw an hour ago. Rotation is deterministic, by
     day number, so it walks the whole back-catalogue rather than re-testing
     whichever topic happens to be handy. */
  (function(){
    var every=Number(P.retentionEvery)||0, done=[];
    if(every>0 && day.n && day.n%every===0 && B && B.topics && day.slots && day.slots.length){
      var curPhase=Math.min.apply(null,day.slots.map(function(s){ return B.topics[s.ti].phase; }));
      done=B.topics.filter(function(t){ return t.phase<curPhase; })
                   .sort(function(a,b){ return a.phase-b.phase || a.i-b.i; });
    }
    if(done.length){
      var pick=done[Math.floor(day.n/every)%done.length];
      add("recall","Blank-page recall \u2014 "+pick.n, 10, pick.i,
        "Not today's work. Phase "+pick.phase+" closed a while ago and has not been opened since "+
        "— write what you still have of "+pick.n+" from memory, then check it against your own misses. "+
        "Thin, but it is the difference between a first pass that holds and one that has to be redone.", null);
    } else {
...
/* ---- Source-data integrity (v15.4) -------------------------------------
   Every hour this app schedules is derived from counts that were read off
   another platform's screens. One bad number does not produce a visible error;
   it produces a plan that is quietly wrong for months. The clearest tell is a
   question count wildly out of proportion to the lectures that teach the same
   material, because the two were captured separately.

   This reports, it does not correct. The ledger is keyed by question NUMBER,
   so silently rewriting a count would orphan logged attempts """+D+""" and I have
   no way to know which of the two numbers is the wrong one. What it can do is
   put the arithmetic in front of the person who can check it in ten seconds.
*/
function dataAnomalies(P){
  P=merge2(DEFAULTS,P||{});
  /* Bank questions per lecture, not (bank+speed) per lecture: the speed quizzes
     come from a different platform and have no lectures behind them, so folding
     them in flags every GI topic and the signal disappears into the noise.

     Outliers are measured with a median/MAD z-score on the LOG ratio rather
     than "4x the median", because the honest spread here is genuinely wide
     (9 to 100 questions per lecture) and a flat multiple flagged eight topics,
     six of which are simply deep GI banks. On log-MAD exactly one topic clears
     3.0, and it is the one already suspected of an OCR slip. A detector that
     cries wolf about Esophagus is a detector that gets ignored. */
  var rows=CURRICULUM.filter(function(t){ return t && t.nlec>0 && t.dtq>0; })
    .map(function(t){ return {t:t, ratio:t.dtq/t.nlec}; });
  if(rows.length<10) return [];
  function med(a){ var s=a.slice().sort(function(x,y){return x-y;}); return s[Math.floor(s.length/2)]; }
  var logs=rows.map(function(r){ return Math.log(r.ratio); });
  var m=med(logs), mad=med(logs.map(function(x){ return Math.abs(x-m); })) || 1e-9;
  var medRatio=Math.exp(m);
  var perQ=P.bankPace*(1+PASS_PACE[1]+PASS_PACE[2]) + (P.anaPerQ||1.2)*passFactors(P.passes||3).a*0.85;
  var out=[];
  rows.forEach(function(r){
    var z=(Math.log(r.ratio)-m)/(1.4826*mad);
    if(z<3) return;
    var t=r.t, expect=Math.round(medRatio*t.nlec), excess=Math.max(0,t.dtq-expect);
    out.push({ topicId:t.i, topic:t.n, phase:t.phase, bank:t.dtq, speed:t.spq, lectures:t.nlec,
      ratio:Math.round(r.ratio), medianRatio:Math.round(medRatio), z:Math.round(z*100)/100,
      expected:expect, excess:excess, hours:Math.round(excess*perQ/60),
      /* A factor-of-ten capture slip is the likeliest single explanation. */
      tenth:(t.dtq>=1000 && Math.abs(Math.round(t.dtq/10)-expect)<Math.abs(t.dtq-expect))?Math.round(t.dtq/10):null });
  });
...
function specialDay(c,P){
  ...
  else if(c.kind==="orient") L=[
    ["check","Set the app up",20,
     "Settings: confirm the exam dates, the day start, and which day model is your default. Nothing else on this list matters if the dates are wrong.",null],
    ["check","Walk the map once",25,
     "Learn tab: seven phases, 39 topics. You are not learning anything today — you are finding out where things live, so tomorrow opens with work instead of navigation.",null],
    ["check","Log 10 questions, any topic",40,
     "Not to score well. The engine has no evidence about you yet, and ten honest attempts — including the ones you guess — are what it uses to decide where to point you first.",null],
    ["check","Write one line about where you actually are",10,
     "Which topics you already know are weak. Future you will not remember today\u2019s honest answer.",null],
    ["check","Take a backup",5,
     "Ten seconds, and it establishes the habit on the one day with nothing to lose.",null],
    ["check","Stop.",0,
     "Orientation is the whole job today. Week one is deliberately light — starting heavy is how a long campaign breaks in week two.",null]];
  ...
  plan.CAL.forEach(function(c,ci){
    ...
    if(isContent){
      ...
      var NAMES={ rest:"Rest & review — no new content", backup:"Weekly backup — 6 protected hours",
        orient:"Orientation — set up, look around, log ten questions",
...
return { CURRICULUM:CURRICULUM, STYLES:STYLES, REPAIR_MIN_GAP:REPAIR_MIN_GAP,
  ...
  loopQueue:loopQueue, CRITERION:CRITERION, specialDay:specialDay, dataAnomalies:dataAnomalies,
